    ...
    for epoch in range(model.start_epoch + 1, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(dataset_train):
            ...
            if opt.save_imgs:
                ...
                # save_dir = '%s/%s.jpg' % (folder_dir, os.path.basename(data['fname'][0]).split('.')[0] + '_data_out')
                # raw = res['data_out'][0].cpu().permute(1, 2, 0).numpy()
                # raw = pack_rggb_channels(raw)                
                # raw_dm = postprocess_raw(demosaic(raw))
                # save_rgb(raw_dm, save_dir)


                # save_dir = '%s/%s.jpg' % (folder_dir, os.path.basename(data['fname'][0]).split('.')[0] + '_raw')
                # raw = res['data_raw'][0].cpu().permute(1, 2, 0).numpy()
                # raw = pack_rggb_channels(raw)
                # raw_dm = postprocess_raw(demosaic(raw))
                # save_rgb(raw_dm, save_dir)


                # save_dir = '%s/%s.jpg' % (folder_dir, os.path.basename(data['fname'][0]).split('.')[0] + '_raw_fusion')
                # raw = res['raw_fusion'][0].cpu().permute(1, 2, 0).numpy()
                # raw = pack_rggb_channels(raw)
                # raw_dm = postprocess_raw(demosaic(raw))
                # save_rgb(raw_dm, save_dir)

                # save_dir = '%s/%s.jpg' % (folder_dir, os.path.basename(data['fname'][0]).split('.')[0] + '_raw_warp')
                # raw = res['raw_warp'][0].cpu().permute(1, 2, 0).numpy()
                # raw = pack_rggb_channels(raw)
                # raw_dm = postprocess_raw(demosaic(raw))
                # save_rgb(raw_dm, save_dir)

                # save_dir = '%s/%s.jpg' % (folder_dir, os.path.basename(data['fname'][0]).split('.')[0] + '_data_out_mask')
                # raw = res['data_out_mask'][0].cpu().permute(1, 2, 0).numpy()
                # raw = pack_rggb_channels(raw)
                # raw_dm = postprocess_raw(demosaic(raw))
                # save_rgb(raw_dm, save_dir)

                # save_dir = '%s/%s.jpg' % (folder_dir, os.path.basename(data['fname'][0]).split('.')[0] + '_data_raw_mask')
                # raw = res['data_raw_mask'][0].cpu().permute(1, 2, 0).numpy()
                # raw = pack_rggb_channels(raw)
                # raw_dm = postprocess_raw(demosaic(raw))
                # save_rgb(raw_dm, save_dir)
                ...
                # save_dir = '%s/%s.jpg' % (folder_dir, os.path.basename(data['fname'][0]).split('.')[0] + '_GCMModel_out_fusion')
                # dslr = res['GCMModel_out_fusion'][0].cpu().permute(1, 2, 0).numpy();
                # save_rgb(dslr, save_dir)

                # save_dir = '%s/%s.jpg' % (folder_dir, os.path.basename(data['fname'][0]).split('.')[0] + '_data_dslr_mask')
                # dslr = res['data_dslr_mask'][0].cpu().permute(1, 2, 0).numpy();
                # save_rgb(dslr, save_dir)
                ...
                # save_dir = '%s/%s.jpg' % (folder_dir, os.path.basename(data['fname'][0]).split('.')[0] + '_GCMModel_out_mask')
                # dslr = res['GCMModel_out_mask'][0].cpu().permute(1, 2, 0).numpy();
                # save_rgb(dslr, save_dir)

                # save_dir = '%s/%s.jpg' % (folder_dir, os.path.basename(data['fname'][0]).split('.')[0] + '_rgb_best_mask')
                # dslr = res['rgb_best_mask'][0].cpu().permute(1, 2, 0).numpy();
                # save_rgb(dslr, save_dir)

                # save_dir = '%s/%s.jpg' % (folder_dir, os.path.basename(data['fname'][0]).split('.')[0] + '_raw_best_mask')
                # dslr = res['raw_best_mask'][0].cpu().permute(1, 2, 0).numpy();
                # save_rgb(dslr, save_dir)
