...
class query_Attention(nn.Module):
    def __init__(self, dim, num_heads=2, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.q = nn.Parameter(torch.ones((1, 17, dim)), requires_grad=True)
    ...
    def forward(self, x):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, 17, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class query_SABlock(nn.Module):
    ...
    def forward(self, x):
        x = x + self.pos_embed(x);print(x.shape)
        x = x.flatten(2).transpose(1, 2);print(x.shape)
        x = self.drop_path(self.attn(self.norm1(x)));print(x.shape)
        x = x + self.drop_path(self.mlp(self.norm2(x)));print(x.shape)
        return x
...
class Global_pred(nn.Module):
    def __init__(self, in_channels=3, out_channels=64, num_heads=4, type='exp'):
        ...
        self.color_base = nn.Parameter(torch.eye((4)), requires_grad=True)  # basic color matrix
        # main blocks
    ...
    def forward(self, x):
        ...
        #print(x.shape)
        color = self.color_linear(color).squeeze(-1).view(-1, 4, 4) + self.color_base
        return gamma, color
